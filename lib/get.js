...
import { isEmpty, isArray, forEach, map, every, reject, keys, flatten, mapKeys, camelCase } from 'lodash'
const fetch = require('node-fetch')

import { isValidNodeURI } from './utils/network'
import { isSecureOrigin, isValidUUID, promiseMap, isValidProofHandle } from './utils/helpers'
...
async function getProofs(proofHandles) {
  ...
  if (!isArray(proofHandles)) throw new Error('proofHandles arg must be an Array')
  if (isEmpty(proofHandles)) throw new Error('proofHandles arg must be a non-empty Array')
  if (
  ...
  try {
    ...
    const parsedBody = await promiseMap(nodesWithGetOpts, fetch, {
      concurrency: 25
    })
