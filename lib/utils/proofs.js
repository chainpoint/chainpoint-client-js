...
import { isEmpty, isString, has, isObject, forEach, map, isBuffer } from 'lodash'
...
export function normalizeProofs(proofs) {
  ...
  let normalized = map(proofs, proof => {
  ...
  // remove any empty or undefined items in the array
  return normalized.filter(proof => proof)
}
