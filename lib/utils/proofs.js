...
export function isValidProofHandle(handle) {
  if (!isEmpty(handle) && isObject(handle) && has(handle, 'uri') && has(handle, 'proofId')) return true
  return false
}
...
export function normalizeProofs(proofs) {
  ...
  forEach(proofs, proof => {
    if (isObject(proof) && has(proof, 'proof') && isString(proof.proof)) {
      ...
      normalized.push(proof)
    } else if (isObject(proof) && !proof.proof && has(proof, 'proofId')) {
      console.error(`no proof for hashIdNode ${proof.proofId}`)
    } else {
...
export function flattenProofs(parsedProofs) {
  ...
  forEach(parsedProofs, parsedProof => {
    ...
    forEach(proofAnchors, proofAnchor => {
      ...
      flatProofAnchor.proof_id = parsedProof.proof_id
      flatProofAnchor.hash_received = parsedProof.hash_received
...
export function flattenBtcBranches(proofBranchArray) {
  ...
  forEach(proofBranchArray, proofBranch => {
    let btcAnchor = {}
    btcAnchor.proof_id = proofBranch.proof_id
    ...
    if (proofBranch.branches) {
      forEach(proofBranch.branches, branch => {
        ...
        if (branch.branches) {
          ...
          // get the btc anchor
          let anchor = btcBranch.anchors.find(anchor => anchor.type === 'btc' || anchor.type === 'tbtc')
